...
| 지표 | 현재 | 목표 | 측정 방법 |
|---|---|---|---|
| 담당자 활성 사용률 | 0% (신규 도구) | 파일럿 참여 담당자 중 주 1회 이상 챗봇 사용 비율 80% 이상 | 챗봇 로그의 사용자별 세션 빈도 ÷ 참여 담당자 수 |
| 챗봇 자가해결률 | - | 담당자 개입 없이 종료 60% 이상 | 챗봇 세션 로그 |
| LLM 오답·오인용 건수 | - | 담당자 검증 기간 중 0건 | 담당자 검수 결과 |

*담당자 활성 사용률·자가해결률은 시스템 로그로 자동 집계됩니다. LLM 오답·오인용 건수만 담당자 검수가 필요합니다.*
...
| 프로젝트 3 (범위 밖) | Phase 2 이후 | 국외출장 관리 — 출장지 기준 출장비 정확 계산, 규정·지침 부합 검증, 시행문 초안 작성 |

*프로젝트 2·3은 본 챗봇과 같은 규정 검색·라우팅 코어를 공유하며, 둘 다 "제출 내용 검증 → 시행문 초안 생성" 패턴을 따르는 별도 툴로 구상 중입니다. 이번 PRD 범위 밖이라 추후 별도 문서로 스코핑합니다.*
