...
| 무근거 응답 발생 건수 | - | 항상 0건 (근거 없이 답변 시 즉시 알람) | 로그 자동 집계 (인용 조항 없는 LLM 응답 카운트) |
| 재질문율 | - | baseline 확보 후 목표치 확정, 우선 추이 모니터링 | 로그 자동 집계 (세션 내 유사 질문 재발생 탐지) |

*LLM 오답·오인용 건수는 담당자 검수가 필요한 지표이고, 나머지 5개는 시스템 로그만으로 자동 집계됩니다. 무근거 응답 발생 건수·재질문율은 물량이 늘어 전수 검수가 어려워지는 Phase 2 이후 조기경보 용도로 특히 유용합니다.*
...
| 프로젝트 2 (범위 밖) | Phase 2 이후 | 사택계약서 검토 + 시행문 자동생성 |
| 프로젝트 3 (범위 밖) | Phase 2 이후 | 국외출장 관리 — 출장비 산출 및 규정 부합 검증 |

*프로젝트 2·3은 본 챗봇과 같은 규정 검색·라우팅 코어를 공유하는 별도 검증형 툴로 구상 중이며, 이번 PRD 범위 밖이라 추후 별도 문서로 스코핑합니다.*
